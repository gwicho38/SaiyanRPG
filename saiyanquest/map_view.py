...
from saiyanquest.camera import project
from saiyanquest.graphics import ColorLike, apply_cinema_bars, load_and_scale
from saiyanquest.map import get_pos_from_tilepos, proj
from saiyanquest.math import Vector2
from saiyanquest.surfanim import SurfaceAnimation, SurfaceAnimationCollection

logger = logging.getLogger(__name__)

if TYPE_CHECKING:
    from saiyanquest.client import LocalPygameClient
    from saiyanquest.db import NpcTemplateModel
    from saiyanquest.map import SaiyanQuestMap
...
class MapRenderer:
    ...
    def draw(self, surface: Surface, current_map: SaiyanQuestMap) -> None:
    ...
    def _prepare_map_rendering(self, current_map: SaiyanQuestMap) -> None:
    ...
    def _get_and_position_surfaces(
        self, current_map: SaiyanQuestMap
    ...
    def _position_surfaces(
        self, current_map: SaiyanQuestMap, surfaces: list[WorldSurfaces]
    ) -> list[tuple[Surface, Rect, int]]:
...
class DebugRenderer:
    ...
    def draw_debug(self, current_map: SaiyanQuestMap, surface: Surface) -> None:
    ...
    def _draw_events(self, current_map: SaiyanQuestMap, surface: Surface) -> None:
    ...
    def _draw_collision_tiles(
        self, current_map: SaiyanQuestMap, surface: Surface
    ) -> None:
...
def collision_box_to_pgrect(
    current_map: SaiyanQuestMap, box: tuple[int, int]
) -> Rect:
    """
...
def npc_to_pgrect(current_map: SaiyanQuestMap, npc: NPC) -> Rect:
